...
**Stack:** PyMuPDF + DocLayout-YOLO + a Gemini-class VLM for the paid
lane (scanned/broken pages only) — the free lane stays fully local.
...
This document is the design as it stood at the end of phase 8, kept for
its revision trail — the trade-offs are the point of this repo. Two
later revisions supersede parts of it:

* **Stage 5 is no longer Tesseract.** Real bilingual corpora broke both
  the OCR engine and the "no LLM tier" decision of §7;
  [gemini_extractor_spec.md](gemini_extractor_spec.md) carries the
  diagnosis and the replacement (one VLM code path for every script).
  §6 and §7's Tesseract-era mechanics below are historical.
* **A clean-room v2 architecture** distilled from the full edge-case
  ledger lives in [rewrite_design.md](rewrite_design.md)
  (`src/rag_ingest2/`).

It originally started from a spec that used a vision LLM for scanned
pages and tables; review dropped the LLM (§7), and the real corpus put
it back in a narrower, better-guarded role — that full circle is the
most instructive arc in the ledger.
...
| **Gemini VLM** (paid lane, was Tesseract) | re-read scanned/broken pages from pixels as markdown | run on pages the free lane already handled |
...
| 5 VLM | `vlm_extract.py` | scanned/rerouted pages via the paid VLM lane | `stages/05_vlm.jsonl`, `05_vlm_pages.json` |
...
## 6. STAGE 5 — was OCR; now the VLM lane

**Superseded** by [gemini_extractor_spec.md](gemini_extractor_spec.md):
stage 5 is `vlm_extract.py` — scanned and broken-CMap pages are re-read
from pixels by a Flash-class VLM behind a pluggable client seam, with a
response cache, deterministic markdown→Units parsing, and a
plausibility-based verification gate (VLMs fail as fluent lies, not
symbol soup).

For the record, the Tesseract design this replaced: PyMuPDF's bundled
libtesseract via `get_textpage_ocr()`, whose textpage flowed through the
stage-2 walk unchanged. It worked for clean English print and failed on
real Devanagari (glued words, Latin shrapnel) — per-language traineddata
and per-script quality heuristics don't scale to a truly multilingual
corpus, which is what forced the revision.

---

## 7. STAGE 6 — Tables: the tiered ladder (and why there is no LLM tier)

> **Revision note:** the "no LLM tier" decision below was reversed by
> [gemini_extractor_spec.md](gemini_extractor_spec.md) after the real
> corpus surfaced lying text layers and multilingual scans tier 2 could
> not read. Tier 2 (pixel grid + OCR) is deleted; scanned-page tables
> now arrive inline from the stage-5 VLM as markdown and pass through
> the same validation/stitching. The hallucination argument below is
> answered by the VLM verification gate, not dismissed — it is why that
> gate exists.
...
(Since the VLM revision: `google-genai` joins the pins, and the paid
lane needs `GEMINI_API_KEY` — the free lane still runs with no network
